...
export const Profil = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        setError({
          fetch: "Une erreur est survenue lors de la récupération des données.",
        });
      }
    };
  ...
  const handleUpdate = async (event) => {
    ...
    if (password && !validatePassword(password)) {
      newErrors.password =
        "Le mot de passe doit être entre 6 et 15 caractères, incluant un caractère spécial.";
    }
  ...
  const handlePasswordChange = async (event) => {
    ...
    if (newPassword !== confirmNewPassword) {
      setError({
        password: "Les nouveaux mots de passe ne correspondent pas.",
      });
      return;
    }
    ...
    if (!validatePassword(newPassword)) {
      setError({
        password:
          "Le nouveau mot de passe doit contenir entre 6 et 15 caractères, y compris un caractère spécial.",
      });
      return;
    }
    ...
    try {
      ...
      setError({
        password: "Une erreur est survenue lors de la modification de l'email.",
      });
    }
  };
  ...
  const handleEmailChange = async (event) => {
    ...
    try {
      ...
      setError({
        email: "Une erreur est survenue lors de la modification de l'email.",
      });
        ...
        <LeftColumn>
          <FormLeft>
            ...
            <InputInfo>
              ...
              />
              {error.location && (
                <div style={styles.error}>{error.location}</div>
              )}
